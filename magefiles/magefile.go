...
func getVersion() string {
	...
	v, _ := sh.Output("gobump", "show", "-r", BUILD_PATH)
	return v
}
...
func Build() error {
	...
	cmd := exec.Command("go", "build", "-trimpath", "-ldflags="+BUILD_LDFLAGS, "-o", bin, BUILD_PATH)
	return cmd.Run()
}
...
func Install() error {
	...
	cmd := exec.Command("go", "install", "-ldflags="+BUILD_LDFLAGS, BUILD_PATH)
	return cmd.Run()
}
